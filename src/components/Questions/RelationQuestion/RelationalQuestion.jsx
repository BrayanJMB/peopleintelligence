...
export const RelationalQuestions = (props) => {
  ...
  const removePreguntaOpcionInput = (index) => {
    const newCustomOptions = props.information.customOptions.filter(
      (_, i) => i !== index
    );

    // Actualiza opcionesInputs eliminando el elemento en el índice dado
    const newOpcionesInputs = props.information.opcionesInputs.filter(
      (_, i) => i !== index
    );
  ...
  return (
    <LayoutQuestion {...props}>
      ...
      <Grid container spacing={2}>
        <Grid item xs={6}>
          {props.information.customOptions.map((_, key) => (
            <div
              key={key}
              style={{
                display: "flex",
        ...
        <Grid item xs={6}>
          {props.information.opcionesInputs.map((_, key) => (
            <div
              key={key}
              style={{
                display: "flex",
            ...
            >
              <TextField
                ...
                variant="outlined"
                style={{ marginTop: 8, marginBottom: 8 }}
                value={props.information.opcionesInputs[key]}
